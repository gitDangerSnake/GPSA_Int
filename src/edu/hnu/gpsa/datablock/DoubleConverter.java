...
public class DoubleConverter implements BytesToValueConverter<Double> {
	...
	@Override
	public Double getValue(byte[] array, int left, int right) {
		long x = ((long) (array[left] & 0xff) << 56) + ((long) (array[left+1] & 0xff) << 48) + ((long) (array[left+2] & 0xff) << 40) + ((long) (array[left+3] & 0xff) << 32)
